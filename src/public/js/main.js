...
Promise.all([getData(songAPI), getData(singerAPI), getData(playlistAPI), getData(videoAPI), getData(songUSUKAPI), getData(songEDMAPI), getData(rankTableAPI)]).then(([songs, singers, playlists, videos, songsUSUK, songsEDM, ranksTable]) => {
...
}).then(() => app.start()).then(() => console.log(rankTableData.data.song.length)).catch((err) => alert(err));
            ...
            return `
                <div class="playlist__mv-item">
                    <div class="playlist__mv-item--video">
                        <video src=${mv.path} width="330" height="186" poster=${mv.poster}> </video>
                    </div>
    ...
    handleEvent: ()=>{
        ...
        btnNext.onclick = () => {
            if(app.isRandom){
                ...
                app.currentIndex >= maxLength ? app.currentIndex = 0 : app.currentIndex++
                if(app.isPlayPersonalSong) {
                    Array.from($$('.personal__song--list-item')).forEach((item)=>{
                        if(item.classList.contains('active')){
                            item.classList.remove('active')
